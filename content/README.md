...
fastapi-mvc available under: http://content.192.168.49.2.nip.io/
...
...
Kubernetes cluster ready
FastAPI available under: http://content.192.168.49.2.nip.io/
You can delete dev-env by issuing: make clean
vagrant@ubuntu-focal:/syncd$ kubectl get all -n content
NAME                                                     READY   STATUS    RESTARTS   AGE
pod/content-7f4dd8dc7f-p2kr7                1/1     Running   0          55s
...
vagrant@ubuntu-focal:/syncd$ curl http://content.192.168.49.2.nip.io/api/ready
{"status":"ok"}
```
...
To bootstrap local minikube Kubernetes cluster exposing `content` application:
```shell
make dev-env
```
...
```shell
$ content --help
Usage: content [OPTIONS] COMMAND [ARGS]...
...
Commands:
  serve  content CLI serve command.
$ content serve --help
Usage: content serve [OPTIONS]
...
```shell
$ make image
$ podman run -dit --name content -p 8000:8000 content:$(cat TAG)
f41e5fa7ffd512aea8f1aad1c12157bf1e66f961aeb707f51993e9ac343f7a4b
$ podman ps
CONTAINER ID  IMAGE                                 COMMAND               CREATED        STATUS            PORTS                   NAMES
f41e5fa7ffd5  localhost/content:0.1.0  /usr/bin/fastapi ...  2 seconds ago  Up 3 seconds ago  0.0.0.0:8000->8000/tcp  content
$ curl localhost:8000/api/ready
{"status":"ok"}
```
...
All application configuration is available in `content.config` submodule.
...
| FASTAPI_PROJECT_NAME | `"content"`                               | FastAPI project name.                                          |
...
1. Source: `content.config/gunicorn.conf.py`
2. [Gunicorn configuration file documentation](https://docs.gunicorn.org/en/latest/settings.html)

### Routes definition

Endpoints are defined in `content.app.router`. Just simply import your controller and include it to FastAPI router:

```python
from fastapi import APIRouter
from content.app.controllers.api.v1 import ready
...
You can implement your own web routes logic straight away in `content.app.controllers.api.v1` submodule. For more information please see [FastAPI documentation](https://fastapi.tiangolo.com/tutorial/).

### Utilities

For your discretion, I've provided some basic utilities:
* RedisClient `content.app.utils.redis`
* AiohttpClient `content.app.utils.aiohttp_client`
...
Example:
```python
from content.app.utils import RedisClient

response = RedisClient.get("Key")
```
```python
from content.app.utils import AiohttpClient
...
Source: `content.app.exceptions.http.py`
