...
minikube image load "mawazo:${TAG}"

echo "[dev-env] creating mawazo namespace"
kubectl create namespace mawazo

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl get namespace mawazo -n mawazo"
...
kubectl create -f manifests/persistent-storage-no-pvc-deletion.yaml -n mawazo

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment.apps/rfs-redisfailover-persistent-keep -n mawazo"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
STATEFULSET_REPLICAS=$(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.spec.replicas}' -n mawazo)
ATTEMPTS=0
until [[ ${STATEFULSET_REPLICAS} -eq $(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.status.readyReplicas}' -n mawazo) ]] || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done

echo "[dev-env] installing mawazo charts"
helm upgrade --install \
    mawazo charts/mawazo \
    --namespace mawazo \
    --set ingress.host.name="mawazo.${INGRESS_HOST}"

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment/mawazo -n mawazo"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
FastAPI available under: http://mawazo.${INGRESS_HOST}/
