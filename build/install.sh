...
Now you should access CLI script: $ mawazo --help
Alternatively you can access CLI script via poetry run: $ poetry run mawazo --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_MAWAZO_COMPLETE=source_bash mawazo)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_MAWAZO_COMPLETE=source_zsh mawazo)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_MAWAZO_COMPLETE=source_fish mawazo)' >> ~/.config/fish/completions/mawazo.fish
EOF